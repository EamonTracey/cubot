...
struct ColorDetector::ONNXImpl {
    ...
    ONNXImpl()
        ...
        session = std::make_unique<Ort::Session>(env, models_linear_svc_onnx,
                                                 models_linear_svc_onnx_len,
                                                 session_options);
    }
};
...
enum Color ColorDetector::DetectColor(const std::vector<float> &image) const {
    return DetectColor(image.data(), image.size());
}
...
enum Color ColorDetector::DetectColor(const float *pixel_data,
    ...
    std::vector<float> features =
        ExtractFeatureVector(pixel_data, pixel_data_len);
...
std::vector<float> ColorDetector::ExtractFeatureVector(const float *pixel_data,
    ...
    size_t n_pixels = pixel_data_len / 3;
    ...
    for (size_t i = 0; i < n_pixels * 3; i += 3) {
        auto pixel =
    ...
    float l_stdev = std::sqrt(l_square_sum / n_pixels - l_mean * l_mean);
    float a_stdev = std::sqrt(a_square_sum / n_pixels - a_mean * a_mean);
    float b_stdev = std::sqrt(b_square_sum / n_pixels - b_mean * b_mean);

    return {l_mean, a_mean, b_mean, l_stdev, a_stdev, b_stdev};
}

/* CHAT-GPT GENERATED BEGIN */
std::vector<float> ColorDetector::RGBToLAB(float r, float g, float b) {
    ...
    auto gamma_correct = [](float c) {
        return (c > 0.04045f) ? std::pow((c + 0.055f) / 1.055f, 2.4f)
                              : c / 12.92f;
    };
